...
def test_chart_builder_guardian_damage_aggregation(auth_client, player, aggregation, expected) -> None:
    """Chart Builder honors aggregation overrides for guardian damage."""

    from analysis.raw_text_metrics import extract_raw_text_metrics
    from gamedata.models import BattleReport, BattleReportDerivedMetrics, BattleReportProgress

    for idx, value in enumerate((100, 300), start=1):
        report = BattleReport.objects.create(
            player=player,
            raw_text=f"Battle Report\nDamage\t{value}\n",
            checksum=f"guardian-{idx}".ljust(64, "x"),
        )
        BattleReportProgress.objects.create(
            battle_report=report,
            player=player,
    ...
    panels = {p["id"]: p for p in json.loads(response.context["chart_panels_json"])}
    builder_panel = panels["chart_builder_custom"]
    assert builder_panel["datasets"][0]["data"] == [expected]
