...
def test_chart_guardrail_limits_data_points(client) -> None:
    ...
    for idx in range(405):
        reports.append(
            BattleReport(raw_text="Battle Report\nCoins earned\t1\n", checksum=f"guardrail{idx}".ljust(64, "x"))
        )
    BattleReport.objects.bulk_create(reports)

    progresses = []
    for idx, report in enumerate(BattleReport.objects.filter(checksum__startswith="guardrail").order_by("id")):
        progresses.append(
    ...
    response = client.get("/", {"charts": ["coins_earned"], "start_date": "2025-01-01"})
