...
**** TODO [#A] URL redirection from remote source (Blocking) 
There are 51 detected occurances... Apply the fix as a global function, fix where `return redirect(redirect_to)` appears, and suggest a rule for agents.md to strictly enforce using the fix. 
...
**** TODO [#A] UW/Guardian Chip/Bots Dashboards (Blocking)
...
**** TODO [#B] Wiki Population (v0.2.0x)
...
**** TODO [#C] Normalize time handling everywhere (game time vs real time vs accelerated) (v0.2.0x)
...
**** TODO [#C] Revise project_structure.md (v0.2.0x)
It's outdated and needs expansion

**** TODO [#C] Move ##Test taxonomy & markings to a dedicated testing page (v0.2.0x)
