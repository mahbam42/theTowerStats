# Bots

## Overview

The Bots page helps you track which bots are unlocked, how far each upgrade parameter has progressed, and how often each bot appears in your imported Battle Reports. Bot names and descriptions are based on the [Bots](https://the-tower-idle-tower-defense.fandom.com/wiki/Bots) page on the The Tower Idle Tower Defense Fandom Wiki (external).

## When to Use This

- You want to confirm which bots are unlocked in your tracker.
- You want to record upgrade progress for a bot’s parameters.
- You want to compare total Medals invested across different bots.
- You want to review whether a bot appears in your imported runs.
- You want to review the bot respec cost and limits.

## How to Use

1. Select **Bots** in the navigation.
2. Select **Show** to filter to **Unlocked only** or **Locked only**.
3. Review a bot’s **Status** to confirm whether it is unlocked.
4. (Optional) Select **Wiki** next to a bot name to open its external wiki page.
5. Select **Unlock** on a locked bot to mark it as unlocked in the app.
6. Select **Details** on an unlocked bot to view its upgrade parameters.
7. Select **Level Up** on a parameter to increase its level by 1.
8. Select **Level Down** on a parameter to reduce its level by 1.
9. Select **Bot Respec (300 Gems)** after you respec in-game when you want the app to reset the tracked Bot state for the current event window.

## How to Read the Results

- **Total Medals invested** is the sum of the upgrade costs for the levels above 0 that you have recorded in the app.
- **Runs used** counts how many of your imported Battle Reports include bot metrics with values greater than 0.
- **Level** is your saved upgrade level for that parameter.
- Level 0 means no upgrades have been recorded yet.
- **Current** is the current raw value for your saved level.
- **Next** shows the next-level value, with the change emphasized in parentheses when it can be parsed.
- **Cost** shows the next-level upgrade cost (informational only).
- **MAX** indicates the parameter has reached the highest known level and cannot be increased further in the app.
- **Bot Respec** summarizes the respec cost, availability, what is refunded, and whether the current event window's respec has already been recorded.
- When Bot Respec is used in the app, all tracked Bots are returned to the locked state, Bot parameter levels are reset to 0, and Bot goals are cleared.

## Notes & Limitations

> **Note**
> Upgrades currently assume sufficient Medals. Costs are informational only, and the app does not check affordability.

> **Note**
> Bots do not have an active or inactive toggle. Once a bot is unlocked in the app, it is treated as active.

> **Note**
> Locked bots do not display parameter rows until they are unlocked in the app.

> **Note**
> **Level Down** is available as a safety option if you accidentally record an upgrade you did not intend.

> **Caution**
> Bot Respec is destructive in the app. It locks all tracked Bots, resets Bot parameter levels to 0, and clears Bot goals.

> **Note**
> Bot respecs are still performed in-game first. Use the app action after the in-game respec so the tracked state matches what happened in-game.

> **Note**
> Bot respecs refund medal upgrades and unlocks only; Labs are not refunded.

> **Caution**
> Cooldown and duration values are based on external wiki tables and are shown in seconds. Wiki values can be inaccurate or drift over time.

> **Caution**
> If no Battle Reports have been imported yet, **Runs used** will show “No battles recorded yet”.

> **Note**
> Amplify Bot does not currently expose usage details in Battle Reports, so its **Runs used** value is muted.

> **Note**
> On smaller screens, the upgrade cards use a more compact layout to keep the details readable.

## Advanced Usage

1. Select **Unlocked only** to review only the bots you actively track.
2. Select **Locked only** to focus on bots you have not unlocked in the app yet.
